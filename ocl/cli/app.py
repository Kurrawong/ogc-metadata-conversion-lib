...
@app.command()
def convert(file: str, format: Annotated[Literal["iso3", "umm", "trainingDML"], typer.Option("--format", "-f", help="Input format")], output: str | None = None, dq: str | None = None):
    """Converts a metadata file to ISO 19115-4 JSON"""
    console.print(f"Converting {file}...")
    # ocl.convert.convert(file)
    console.print(oclconvert(file, format).model_dump_json())
